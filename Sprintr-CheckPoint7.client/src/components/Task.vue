...
<template>
  <div class="modal fade" :id="'create'+ btask.id" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
    ...
    return {
      ...
      async createTask() {
        try {
          ...
          state.newTask.backlogId = props.btask.id
          await tasksService.create(state.newTask)
          state.newTask = {}
          Pop.toast('Task created successfuly')
        } catch (error) {
